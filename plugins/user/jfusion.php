	...
	*/
	function onLoginUser($user, &$options)
	{
		...
		$master = JFusionFunction::getMaster();
			...
			//let's see if we can pull the joomla's user plugin
			$JUser = JPATH_PLUGINS.DS.'user'.DS.'joomla.php';
			if(file_exists($JUser)){
				require_once($JUser);
				$JUser = new plgUserJoomla($this->subject,$this->config = $config);
				$result = $JUser->onLoginUser($user,$options);
				return $result;
			}
		}
		ob_end_clean();
