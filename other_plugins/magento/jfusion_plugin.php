...
class JFusionPlugin_magento extends JFusionPlugin{
    ...
    function setupFromPath($forumPath)
    {
        ...
        if (file_exists($xmlfile)) {
            ...
            return $params;
/*
            // get the configuration data into $params
            $file = JPATH_ADMINISTRATOR .'/components/com_jfusion/plugins/magento/jfusion.xml';
            $params = new JParameter('', $file );
            $params->set('source_path', (string)$forumpath);
            $params->set('database_type', "mysql");
            $params->set('database_host', (string)$xml->global->resources->default_setup->connection->host );
            $params->set('database_name', (string)$xml->global->resources->default_setup->connection->dbname);
            $params->set('database_user', (string)$xml->global->resources->default_setup->connection->username);
            $params->set('database_password', (string)$xml->global->resources->default_setup->connection->password);
            $params->set('database_prefix',  (string)$xml->global->resources->db->table_prefix);
