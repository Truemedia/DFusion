   ...
   */
    ...
    */
	...
	function fillMagentoDataObject($entity_type_code, $entity_id, $entity_type_id) {
		...
		for($i = 0; $i < count ( $result ); $i ++) {
			if ($result [$i] ['backend_type'] == 'static') {
				$query = 'SELECT ' . $result [$i] ['attribute_code'] . ' FROM #__' . $entity_type_code . '_entity' . ' WHERE entity_type_id =' . $entity_type_id . ' AND entity_id =' . $entity_id;
				$db->setQuery ( $query );
				if ($db->_errorNum != 0) {
					$result = false;
					return $result;
				...
				$query = 'SELECT value FROM #__' . $entity_type_code . '_entity_' . $result [$i] ['backend_type'] . ' WHERE entity_type_id =' . $entity_type_id . ' AND attribute_id =' . $result [$i] ['attribute_id'] . ' AND entity_id =' . $entity_id;
				$db->setQuery ( $query );
				if ($db->_errorNum != 0) {
					$result = false;
					return $result;
	...
	function updateUser($userinfo, $overwrite) {
		...
		// In Magento, the username is the email address but if it's changed in Joomla, Magento won't be aware
		// So we need to get the old username and update this username if it's changed
		$username = (isset($userinfo->olduserinfo->email) && ($userinfo->olduserinfo->email != $userinfo->email) )?$userinfo->olduserinfo->email:$userinfo->email;
		...
		if (! empty ( $existinguser )) {
			...
			// Update the new email address/username
	...
	function update_create_Magentouser($user, $entity_id) {
		...
		if (! $entity_id) { //create an (almost) empty user
			...
			$query = 'UPDATE #__eav_entity_store SET increment_last_id = ' . $db->Quote ( $increment_last_id ) . ' WHERE entity_type_id = ' . $this->getMagentoEntityTypeID ( 'customer' ) . ' AND store_id = 0';
			$db->Execute ( $query );
			if ($db->_errorNum != 0) {
				$db->Execute ( 'ROLLBACK TRANSACTION' );
				//RollbackTrans();
			...
			$query = 'INSERT INTO #__customer_entity   (entity_type_id, increment_id, is_active, created_at, updated_at) VALUES ' . '(' . $this->getMagentoEntityTypeID ( 'customer' ) . ',' . $db->Quote ( $increment_last_id ) . ',1,' . $db->Quote ( $sqlDateTime ) . ', ' . $db->Quote ( $sqlDateTime ) . ')';
			$db->Execute ( $query );
			if ($db->_errorNum != 0) {
				$db->Execute ( 'ROLLBACK TRANSACTION' );
				//RollbackTrans();
			...
			$query = 'UPDATE #__customer_entity' . ' SET updated_at = ' . $db->Quote ( $sqlDateTime ) . ' WHERE entity_id = ' . $entity_id;
			$db->Execute ( $query );
			if ($db->_errorNum != 0) {
				$db->Execute ( 'ROLLBACK TRANSACTION' );
				//RollbackTrans();
		...
		for($i = 0; $i < count ( $user ); $i ++) {
			if ($user [$i] ['backend_type'] == 'static') {
				...
				if (isset ( $user [$i] ['value'] )) {
					$query = 'SELECT value FROM #__customer_entity' . '_' . $user [$i] ['backend_type'] . ' WHERE entity_id = ' . $entity_id . ' AND entity_type_id = ' . $this->getMagentoEntityTypeID ( 'customer' ) . ' AND attribute_id = ' . $user [$i] ['attribute_id'];
					$db->Execute ( $query );
					$result = $db->loadresult ();
					if ($result) {
						// we do not update an empty value, but remove the record instead
						if ($user [$i] ['value'] == '') {
							$query = 'DELETE FROM #__customer_entity' . '_' . $user [$i] ['backend_type'] . ' WHERE entity_id = ' . $entity_id . ' AND entity_type_id = ' . $this->getMagentoEntityTypeID ( 'customer' ) . ' AND attribute_id = ' . $user [$i] ['attribute_id'];
						}
						else {
							$query = 'UPDATE #__customer_entity' . '_' . $user [$i] ['backend_type'] . ' SET value = ' . $db->Quote ( $user [$i] ['value'] ) . ' WHERE entity_id = ' . $entity_id . ' AND entity_type_id = ' . $this->getMagentoEntityTypeID ( 'customer' ) . ' AND attribute_id = ' . $user [$i] ['attribute_id'];
						}
					}
					else { // must create
						$query = 'INSERT INTO #__customer_entity' . '_' . $user [$i] ['backend_type'] . ' (value, attribute_id, entity_id, entity_type_id) VALUES (' . $db->Quote ( $user [$i] ['value'] ) . ', ' . $user [$i] ['attribute_id'] . ', ' . $entity_id . ', ' . $this->getMagentoEntityTypeID ( 'customer' ) . ')';
					}
					$db->Execute ( $query );
					if ($db->_errorNum != 0) {
						$db->Execute ( 'ROLLBACK TRANSACTION' );
	...
	function createUser($userinfo, &$status) {
		...
		if ($userinfo->activation) {
			$this->fillMagentouser ( $magento_user, 'is_active', $userinfo->activation );// replace confirmation by is_active
		}
...
*/		$this->fillMagentouser ( $magento_user, 'group_id', $default_group_id );
	...
	function updateUsername($userinfo, &$existinguser, &$status) {
		...
		if ($errors) {
			...
			$status ['debug'] [] = JText::_ ( 'USERNAME_UPDATE' ) . ': ' . $existinguser->email . ' -> ' . $userinfo->email;
		}
	}
